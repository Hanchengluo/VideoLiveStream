window.onload = () => {
    ...
    document.getElementById('LiveStream').oncanplaythrough = () => {
        var Video = document.getElementById('LiveStream')
        var videoWidth = Video.videoWidth / Video.videoHeight * document.querySelectorAll('.live')[0].clientHeight
        Module.LivePlayStyle.width = videoWidth - 2 + 'px'
        Module.LivePlayStyle.left =  (document.documentElement.clientWidth / 2) - (videoWidth / 2) + 'px'
        sessionStorage.LivePlayStyleWidth = videoWidth - 2
        sessionStorage.LivePlayStyleLeft = (document.documentElement.clientWidth / 2) - (videoWidth / 2)
        ...
        window.onresize = () => {
            if(FullScreenType == false){
                Module.LivePlayStyle.width = videoWidth + 'px'
                Module.LivePlayStyle.left =  (document.documentElement.clientWidth / 2) - (videoWidth / 2) + 'px'
            }
        }
        // 控制条显示5秒
    ...
    document.getElementById('LiveStreamPlan').onclick = (Event) => {
        ...
        var i = Event.clientX - (Number(Module.LivePlayStyle.left.split('p')[0]))
        var time = Math.ceil(sum * (i / Number(sessionStorage.LivePlayStyleWidth)))
        document.getElementById('LiveStream').currentTime = (time >= sum) ? sum : time
    }
    // 显示进度条
